...
const index = () => {
  ...
  const fetchUserProfile = async () => {
    try {
      const response = await axios.get(
        `https://warm-buses-behave.loca.lt/profile/${userId}`
      );
  ...
  useEffect(() => {
    const fetchAllPosts = async () => {
      try {
        const response = await axios.get("https://warm-buses-behave.loca.lt/all");
  ...
  const handleLikePost = async (postId) => {
    try {
      const response = await axios.post(
        `https://warm-buses-behave.loca.lt/like/${postId}/${userId}`
      );
